 ...
 *   the same connection authenticated via the user bind. If the application is configured to run
 *   in this mode, then a login user is possible to do the search on the connection that was bound
 *   to a different user. If the search capbility in ldap are different for different users, then
 *   this will create a security leak for ldap search.
...
if (ad.ldapsCA) {
  searchClientOptions.tlsOptions = {
    ca: fs.readFileSync('../config/' + ad.ldapsCA)
  };
}
