...
function createRequest(requests, i) {
  ...
  if (!validator.isInt(request[10])) {
    console.log('cannot read the quantity of ' + i + ' line: not a integer');
  ...
  if (namecodes.length !== 3) {
    if (namecodes.length === 0) {
      console.log('cannot encode the name of ' + i + ' line: origin category wrong');
    }
    if (namecodes.length === 1) {
      console.log('cannot encode the name of ' + i + ' line: origin sub category wrong');
    }
    if (namecodes.length === 2) {
      console.log('cannot encode the name of ' + i + ' line: signal classification wrong');
    }
    if (i === requests.length - 1) {
      return jobDone();
    }
